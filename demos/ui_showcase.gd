extends Control
## UIShowcase：UI 积木实验室（项目主场景，F5 运行）。
## 左侧分类（ToggleButton 互斥组，dogfooding），右侧 DemoContainer 动态加载 Demo。
## 新增 Demo：在 DEMO_SCENES 注册 + 侧栏加一个分类按钮。

const DEMO_SCENES: Dictionary = {
	"button": "res://demos/button/ButtonDemo.tscn",
	"panel": "res://demos/panel/PanelDemo.tscn",
	"card": "res://demos/card/CardDemo.tscn",
	"drawer": "res://demos/drawer/DrawerDemo.tscn",
}

@onready var _container: Control = $Root/ContentArea/DemoContainer
@onready var _cat_buttons: Array[Button] = [
	$Root/Sidebar/Margin/VBox/BtnButton,
	$Root/Sidebar/Margin/VBox/BtnPanel,
	$Root/Sidebar/Margin/VBox/BtnCard,
	$Root/Sidebar/Margin/VBox/BtnDrawer,
]
...
func _on_cat_toggled(selected: bool, btn: Button, demo_id: String) -> void:
	if not selected:
		return
	# 互斥：取消其他分类按钮的选中态
	for other: Button in _cat_buttons:
